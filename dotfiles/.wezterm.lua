...
-- GTK renders the titlebar (with working buttons + Whitesur theming).
-- WezTerm renders the tab bar directly below.
-- Both share the same background so they read as one continuous header.
...
-- Derive surface/inactive: lighten/darken BG slightly
...
-- If muted came back as rgba, fall back
if not MUTED or MUTED:match('rgba') then MUTED = '#8c8c8c' end

-- Window frame: titlebar background = tab bar background = seamless blend.
-- Zero-width borders let the GTK theme's rounded corners show through.
config.window_frame = {
  border_left_width = 0,
...
-- Window title shows workspace name + tab position
...
-- Retro tab formatting: title left-aligned, padded to fill full tab width
wezterm.on('format-tab-title', function(tab, tabs, panes, cnf, hover, max_width)
  local title = tab.active_pane.title
  -- Build a tab that fills max_width: left pad + title + right fill
  local left_pad = '  '
  local right_fill = string.rep(' ', math.max(0, max_width - #title - 4))
  return {
    { Text = left_pad .. title .. right_fill .. '  ' }
  }
end)
